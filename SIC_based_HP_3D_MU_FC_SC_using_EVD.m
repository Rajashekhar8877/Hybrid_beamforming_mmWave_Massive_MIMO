...
N_iter=10; % earlier taken for 100 iterations 
...
grid on 
title('NtxNr=128x8, NtRF=16, NrRF=4, L=3, users=4, EVD - [2.7 min]')
...
for user=1:1:Num_users
    channel(:,:)= H(:,:,user);
    [Uu,~,~]=svd(channel); 
%    [Uu1,Du1]=eig(channel); % input of evd should be square
%    [Uu,Du]=sortem(Uu1,Du1);
%    u1=Uu(:,1); %op=v1
...
[~,~,V]=svd(H_eff);
%[V1,D1]=eig(channel); % input of evd should be square 
%[V,D]=sortem(V1,D1);
%v1=V(:,1); %op=v1
...
%    [Uu,~,~]=svd(Su);
%    u1=Uu(:,1); 
    [Uu1,Du1]=eig(Su); % input of evd should be square
    [~,indu]=sort(diag(Du1),'descend');
%    Du=Du1(ind1,ind1);
    Uu=Uu1(:,indu);
    u1=Uu(:,1); %op=v1
...
%    [~,~,V]=svd(S);
%    v1=V(:,1);    
    [V1,D1]=eig(S); % input of evd should be square
